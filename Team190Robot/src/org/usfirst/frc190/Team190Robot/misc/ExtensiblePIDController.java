...
public class ExtensiblePIDController implements IUtility, LiveWindowSendable {
    ...
    private ExtensiblePIDController.Tolerance m_tolerance;	//the tolerance object used to check if on target
    ...
    public class PercentageTolerance implements ExtensiblePIDController.Tolerance {
	double percentage;
    ...
    public class AbsoluteTolerance implements ExtensiblePIDController.Tolerance {
	double value;
    ...
    public class NullTolerance implements ExtensiblePIDController.Tolerance {
    ...
    private class PIDTask extends TimerTask {
        ...
        private ExtensiblePIDController m_controller;

        public PIDTask(ExtensiblePIDController controller) {
            if (controller == null) {
     ...
     */
    public ExtensiblePIDController(double Kp, double Ki, double Kd, double Kf, 
        ...
        m_controlLoop.schedule(new ExtensiblePIDController.PIDTask(this), 0L, (long) (m_period * 1000));

        instances++;
        UsageReporting.report(UsageReporting.kResourceType_PIDController, instances);
	m_tolerance = new ExtensiblePIDController.NullTolerance();
    }
     ...
     * @param period 
     */
    public ExtensiblePIDController(double Kp, double Ki, double Kd,
            PIDSource source, PIDOutput output,
            double period) {
        ...
        if (enabled) {
            ...
            synchronized (this) {
                ...
                for (int i = 0; i < terms.size(); i++) {
                    m_result += ((ControlTerm) terms.elementAt(i)).calculate(m_setpoint, input);
                }
    ...
    public synchronized void setTolerance(double percent) {
        m_tolerance = new ExtensiblePIDController.PercentageTolerance(percent);
    }
     ...
     */
    private void setTolerance(ExtensiblePIDController.Tolerance tolerance) {
	...
	m_tolerance = new ExtensiblePIDController.AbsoluteTolerance(absvalue);
    }
	...
	m_tolerance = new ExtensiblePIDController.PercentageTolerance(percentage);
    }
